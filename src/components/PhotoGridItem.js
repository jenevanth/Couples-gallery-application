...
const PhotoGridItem = ({
  ...
  profile = 'me',
  index = 0,
  suspend = false, // NEW: when true, do not render the Image to free memory
}) => {
  ...
  // When suspending, reset loading/error to avoid overlays lingering
  useEffect(() => {
    if (suspend) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Render the Image only when not suspended */}
          {!suspend && (
            <Image
          ...
          {/* Loading shimmer (only when not suspended) */}
          {!suspend && imageLoading && (
          ...
          {/* Video indicator (optional when not suspended) */}
          {!suspend && isVideo && (
          ...
          {image.favorite && (
            <Animated.View
              style={[
                ...
                { transform: [{ scale: heartAnim }], opacity: heartAnim },
              ]}
              pointerEvents="none"
            >
              <LinearGradient
          ...
          {selected && (
            <Animated.View
              style={[styles.borderGlow, { opacity: selectAnim }]}
              pointerEvents="none"
            >
              <LinearGradient
...
const styles = StyleSheet.create({
  ...
  topGradient: { position: 'absolute', top: 0, left: 0, right: 0, height: 50 },
  videoIndicator: {
    position: 'absolute',
    top: '50%',
    left: '50%',
  ...
  selectContainer: { position: 'absolute', top: 8, left: 8 },
  selectGradient: {
    width: 28,
    height: 28,
    borderRadius: 14,
  ...
  favoriteContainer: { position: 'absolute', top: 8, right: 8 },
  heartGradient: {
    width: 28,
    height: 28,
    borderRadius: 14,
  ...
  badge: { borderRadius: 12, padding: 4 },
  dateContainer: { position: 'absolute', bottom: 8, right: 8 },
  dateBadge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 10 },
  dateText: { color: '#fff', fontSize: 10, fontWeight: '500' },
